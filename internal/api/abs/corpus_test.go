...
func corpusStack(t *testing.T) (*httptest.Server, string, map[string]string) {
	...
	// mirrors the ABS mounting in internal/server/server.go
	app := neutron.New(
	...
	vars := map[string]string{
		"{userId}":   "u-" + strconv.FormatInt(uid, 10),
		"{libId}":    strconv.FormatInt(libID, 10),
		"{itemId}":   strconv.FormatInt(edID, 10),
		"{id}":       strconv.FormatInt(edID, 10),
...
func TestCorpus(t *testing.T) {
	dir := filepath.Join("..", "..", "..", "testcorpus", "abs")
